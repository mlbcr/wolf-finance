...
export default function ReunioesPage() {
    ...
    async function salvarReuniao() {
        try {
            if (!dados.numero || !dados.hora || !dados.data) {
            ...
            setErro('Reunião criada com sucesso!')
            setTimeout(() => {
                fecharModal()
                setErro(null)
            }, 1500)
    ...
    async function gerarQRCode(reuniao) {
        try {
            const blob = await gerarQRCodeReuniao(reuniao.id)

            // Cria um link para download
            const url = URL.createObjectURL(blob)
            const link = document.createElement('a')
            link.href = url
            link.download = `qrcode-reuniao-${reuniao.numero}.png`
            link.click()
    ...
    async function verPresencas(reuniao) {
        try {
            ...
            const presencasData = await listarPresencasReuniao(reuniao.id)
    ...
    async function confirmarDeleteReuniao() {
        try {
            ...
            setErro('Reunião deletada com sucesso!')
            setTimeout(() => {
                setErro(null)
            }, 1500)
        } catch (error) {
            setErro(error.message || 'Erro ao deletar reunião')
        } finally {
            setProcessando(false)
        }
    }
    ...
    if (loading || loadingAuth) {
        return (
            <main className="reunioes-page">
                <LoadingModal mensagem="Carregando reuniões..." />
            </main>
        )
    }
    ...
    return (
        <main className="reunioes-page">
            ...
            {/* MODAIS */}
            {processando && <LoadingModal mensagem="Processando..." />}
            {erro && (
                <AlertModal
            ...
            <header className="reunioes-header">
                <div>
                    ...
                    <h1>Reuniões</h1>
                    <p>Acompanhe as reuniões programadas</p>
                </div>

                {isAdmin && (
                    <button
                        className="btn-nova-reuniao"
            ...
            <section className="reunioes-section">
                <div className="section-header">
                    <div>
                        <h2>Reuniões programadas</h2>
                        <p>Lista de todas as reuniões</p>
                    </div>
                            ...
                            <div key={reuniao.id} className="reuniao-item">
                                <div className="reuniao-info">
                                    <div className="reuniao-numero">
                                        <span className="badge">#{reuniao.numero}</span>
                                    </div>
                                    ...
                                    <div className="reuniao-detalhes">
                                        <div className="reuniao-data-hora">
                                            ...
                                            <span>{formatarHora(reuniao.hora)}</span>
                                        </div>
                                    ...
                                    {isAdmin && (
                                        <>
                                            <button
                                                ...
                                                onClick={() => gerarQRCode(reuniao)}
                                                title="Gerar QR Code"
                                            >
                                                <i className="fa-solid fa-qrcode"></i>
                                            </button>
                                            <button
                                                className="btn-presencas"
                                                onClick={() => verPresencas(reuniao)}
                                                title="Ver presenças"
                                            >
                                            ...
                                            <button
                                                ...
                                                onClick={() => setConfirmDelete(reuniao.id)}
                ...
                <div className="modal-overlay" onClick={fecharModal}>
                    <div className="modal-container" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-footer">
                            ...
                            <button className="btn-salvar" onClick={salvarReuniao}>
                                Criar
                            </button>
                        </div>
                    </div>
            ...
            {/* MODAL PRESENÇAS */}
            {modalPresencas && (
                <div className="modal-overlay" onClick={() => setModalPresencas(false)}>
                    <div className="modal-container" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2>Presenças - Reunião #{reuniaoSelecionada?.numero}</h2>
                            <button className="btn-fechar" onClick={() => setModalPresencas(false)}>
                                <i className="fa-solid fa-times"></i>
                            </button>
                        </div>
                                        ...
                                        <div key={presenca.id} className="presenca-item">
                                            <div>
                                                ...
                                                <p className="presenca-horario">
                                                    {new Date(presenca.registrada_em).toLocaleString('pt-BR')}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="modal-footer">
                            <button className="btn-fechar-modal" onClick={() => setModalPresencas(false)}>
                                Fechar
                            </button>
                        </div>
                    </div>
